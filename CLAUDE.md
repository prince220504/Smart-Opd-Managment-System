...
- [ ] **Step 14** — DRF layer, chunked 14a → 14d, branch `feature/drf-api` → PR #5
  ...
  - [ ] **Step 14b** — Install DRF + first serializer + read-only ViewSet + router (`/api/appointments/`). Concepts: serializer, ViewSet, router, browsable API.
  - [ ] **Step 14c** — SimpleJWT auth (`/api/token/`) + role-based API permissions (mirror view gates).
  - [ ] **Step 14d** — Write operations via API + django-filter + drf-spectacular docs page.
...
### Day 21 resume point — Step 14b (DRF install + first API)

1. Confirm on `feature/drf-api`, clean tree, synced. CLAUDE.md auto-loads.
2. **14b scope (~1:15)**: `pip install djangorestframework` + freeze requirements.txt + `'rest_framework'` in INSTALLED_APPS. `AppointmentSerializer` (ModelSerializer — the ModelForm analogue for JSON). Read-only `AppointmentViewSet` (role-scoped queryset: patient→own, doctor→own, reception→all — reuse the scoping logic pattern). `DefaultRouter` under `/api/`. Browsable API verify.
3. Concepts: serializer vs form, ViewSet vs FBV, router vs urlpatterns, content negotiation.
4. Per [[feedback-incremental-tutorial]]: 14b section appended to `tutorial/14-drf-api.md` + README + roadmap + memory + docs commit.
5. 14c (SimpleJWT) next, then 14d, then PR #5.
