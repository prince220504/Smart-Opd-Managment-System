...
### Day 26 resume point — Step 15d (booking validation) → PR #6

1. Confirm on `feature/availability-and-cancel-reason`, clean tree (Day 25 docs commit landed), synced.
2. **15d — booking validation** (~1 hr): new helper in `appointments/forms.py` (sibling of `_validate_slot_free`) — `_validate_doctor_available(cleaned_data)`:
   - Get doctor's availability for the chosen date: DATE-override row first (`doctor.availabilities.filter(recurrence='DATE', date=appointment_date)`), else the recurring row (`.exclude(recurrence='DATE').first()`).
   - No availability at all → "Doctor has not set a schedule yet" (decide: block or allow — lean BLOCK, consistent).
   - Weekday check: `appointment_date.weekday()` — EVERYDAY any / WEEKDAYS 0-4 / MON_SAT 0-5 / DATE exact-date.
   - Time check: `start_time <= time_slot < end_time` and not inside any break (`b['start'] <= slot < b['end']` — compare as strings HH:MM vs time.strftime, or parse; pick one and be consistent).
   - Fail → `ValidationError("Doctor is not available at this time.")`.
   - Call from BOTH booking forms' `clean()` (after `_validate_slot_free`).
3. **Cheap add**: show doctor's hours text on `book.html` ("Dr X: Weekdays 09:00–18:00, breaks 13:00–14:00") — pass availability in `book_appointment` context.
4. Browser-verify: inside hours → books; in break → error; off-day → error; DATE override day → override hours win.
5. Tutorial 15d + README `✅ Done` + roadmap + memory → docs commit. **Then PR #6** (Step 15 avail+cancel complete). After merge: lab module (`feature/lab-module`).
6. Modes carry. Grep list active (+return-indentation).
