...
- [x] **Step 13** — `appointments` app ✅ COMPLETE (13a–13d, PR #3 merged `49887e7`) — chunked, single branch `feature/appointments-app` (Option A)
...
- [x] **Step 14** — DRF layer ✅ COMPLETE (14a–14e, PR #5 merged `299f45d`) — branch `feature/drf-api`
...
- [x] **Step 15 (availability + cancel-reason)** ✅ COMPLETE (15a–15d, PR #6 merged `57f4f71`) — branch `feature/availability-and-cancel-reason`. **Availability = VALIDATION layer, NOT slots** (patient booking UX unchanged; booking `clean()` validates chosen time vs doctor's hours → "Doctor not available"). Chunked:
  ...
  - [x] **15b — `DoctorAvailability` model** ✅ (Day 25, 2026-07-16). `doctor` FK (CASCADE, related_name='availabilities'), `recurrence` TextChoices (EVERYDAY/WEEKDAYS/MON_SAT/DATE), `date` (null=True, DATE only), `start_time`, `end_time`, `breaks` JSONField(default=list). Admin registered. Migration 0006 — done TWICE via rollback dance (migrate 0005 → delete file → fix → remake): typo #1 class `DoctorAvailablility` (extra "li", propagated to admin+migration filename), typo #2 `related_name='availabilites'`. Admin JSON gotcha: double quotes required (`[{"start":"12:00","end":"13:00"}]`, not single). Commit `033db56`. Concept: JSONField + default=list callable (mutable-default bug).
  - [x] **15c — doctor schedule setup + first-login gate** ✅ (Day 25, 2026-07-16). `DoctorScheduleForm(ModelForm)` — fields recurrence/date/start/end, `clean()` (DATE needs date; end > start). **Unlimited breaks (user upgraded mid-build from 1 → 2 → N)**: breaks NOT form fields — template has one `.break-row` (paired `type=time` inputs sharing names) + 8-line vanilla `cloneNode` JS "+ Add break"; view collects via `request.POST.getlist('break_start'/'break_end')` + zip + skip-empties → JSON. **One-recurring-rule invariant**: saving non-DATE deletes doctor's other non-DATE rows first. Gate in `login_view` DOCTOR branch: `not user.availabilities.exclude(recurrence='DATE').exists()` → redirect `appointments:doctor_schedule`. Route `doctor/schedule/`, nav "My Schedule", template `doctor_schedule.html`. Browser-verified (gate→setup→save 2 breaks→today; re-login lands today). **Day 25 typos (6)**: model-class typo, related_name typo, leftover break form-fields after design change (add-without-delete), `form.erros`/`forms.erros` (silent empty error block), `recureence` in `.exclude()` (FieldError at login), **NEW PATTERN: return-outside-if** — gate's doctor_today return mis-indented outside the DOCTOR check → every role got doctor_today + dead code below; valid Python, `check` passes. Commits `e6a1b32` (backend) + `8badc31` (frontend).
  - [x] **15d — booking validation + schedule view/update** ✅ (Day 26, 2026-07-17). `_validate_doctor_available(cleaned_data)` in forms.py (sibling of `_validate_slot_free`, both called from BOTH booking forms' `clean()`): DATE-override row first (`.filter(recurrence=DATE, date=...)`) else recurring (`.exclude(recurrence=DATE)`), none → "has not set a schedule" (block); weekday check (EVERYDAY all / WEEKDAYS >4 / MON_SAT >5 / DATE exact via filter); window `start <= slot < end` (time objects); breaks via `slot.strftime('%H:%M')` string compare (zero-padded HH:MM sorts chronologically). Doctor hours text on `book.html` (recurring rule in `book_appointment` context; `{{ b.start }}` dict dot-lookup). **Schedule page upgraded (user mid-session)**: view-mode summary block + "Update Schedule" button → `?edit=1` → form PREFILLED (`initial=` + break rows rendered with values, cloneNode JS still works); save redirects back to schedule page; `editing = current is None or GET.edit or POST`. **Day 26 typos (3)**: elif/else/render mis-indented INSIDE POST block (GET → ValueError returned-None + failed-POST would hide errors — return-indentation pattern again), `{'ends': be}` JSON key (KeyError at validation; stale bad rows need re-save), `forms.erros` (3rd occurrence). Commits `447496b` (backend) + `3da9012` (frontend). **STEP 15 (avail+cancel) COMPLETE → PR #6.**
  - **GREEN/RED CALENDAR deferred to Step 21 frontend (user emphasized DO NOT FORGET)**: patient date picker shows available=green/unavailable=red. Native `<input type=date>` can't style dates → needs custom JS calendar + endpoint. The recurrence rule (15b-d) makes it possible; paint in Step 21.
- [ ] **Step 16 — lab module** — branch `feature/lab-module` → PR #7. Split 16a models+admin+media / 16b request+queue / 16c upload+status / 16d PDF. Auth extras (forgot-password, profile photo w/ Pillow) — 16e or later.
  - [x] **16a — models + admin + media** ✅ (Day 27, 2026-07-18). `lab` app scaffold (`apps.py` dotted-path + INSTALLED_APPS). `LabTest`: appointment FK PROTECT (related_name='lab_tests'), requested_by FK PROTECT ('requested_tests'), test_name, status TextChoices REQUESTED/IN_PROGRESS/DONE, requested_at auto_now_add, `Meta.ordering=['requested_at']` (oldest-first = 16b queue order free). `LabResult`: **first OneToOneField** (test → LabTest, CASCADE, related_name='result' — FK+UNIQUE, one result per test schema-enforced, reverse = single object not queryset), uploaded_by FK PROTECT, **first FileField** (`upload_to='lab_results/'` — DB stores path string, bytes under MEDIA_ROOT; no Pillow needed, that's ImageField-only), notes, is_normal Boolean, result_date. Cross-app import `from apps.appointments.models import Appointment` direct (one-directional, no circularity). Admin both models. **Media serving in dev**: `urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)` in config/urls — self-disables when DEBUG=False. Migration `lab.0001_initial`. Browser-verified: admin add test + upload result file → lands in `backend/media/lab_results/`, URL serves, second result for same test blocked (OneToOne). **Day 27 typos (2)**: `class status` lowercase (enum-vs-field casing at DEFINITION side — NameError at check, loud), `ForeingKey` transposed. Commits `acd3f69` (lab app) + `f552368` (media serving).
  - [ ] **16b — doctor request-test + lab queue** — request button on CONFIRMED appointments (doctor pages), creates LabTest with requested_by; lab pending queue page (REQUESTED + IN_PROGRESS, oldest first); LAB role login redirect + nav branch.
  - [ ] **16c — upload result + status flow** — lab upload form (FileField → `request.FILES`, `enctype="multipart/form-data"`), status transitions REQUESTED→IN_PROGRESS→DONE.
  - [ ] **16d — ReportLab PDF** — branded lab report (hospital header, patient/test/result/technician/normal-flag) + FileResponse download for patient + doctor.
- [ ] **Step 17 — prescriptions** — Prescription model (OneToOne Appointment, medicines JSONField, only on COMPLETED), doctor write view (dynamic medicine rows — getlist pattern from 15c), patient view, PDF download.
- [ ] **Step 18 — notifications + background tasks** — Notification model + 8 triggers (matrix) + bell + email (Gmail SMTP). **DECIDE HERE: Celery+Redis vs Django 6 built-in Tasks** (catalogue leans built-in — fewer deps) for 24hr reminder + **USER-REQUESTED (Day 18) auto-transitions**: stale PENDING past slot → CANCELLED/EXPIRED. NOTE: do NOT auto-complete CONFIRMED (confirmed ≠ visited) — humans mark Complete manually (13e-i).
- [ ] **Step 19 — dashboard + exports + medical history** — reception stats (annotate/Count → Chart.js), patient history page (prefetch_related), CSV/Excel exports.
- [ ] **Step 20 — deploy** — Whitenoise static/media in production + production settings (DEBUG off, env vars, PostgreSQL) + Render setup + live URL.
- [ ] **Step 21 — frontend + README (LAST)** — Stitch AI designs → Antigravity conversion to Django templates, **green/red availability calendar (deferred from Step 15 — DO NOT FORGET)**, professional README (demo GIF, badges, Swagger screenshot).
...
### Day 25 recap (2026-07-16) — STEP 15b + 15c SHIPPED (availability model + schedule setup + gate)

- **15b**: `DoctorAvailability` model + admin + migration 0006. Migration done TWICE via the rollback dance (`migrate appointments 0005` → delete 0006 file → fix → remake): typo #1 = class name `DoctorAvailablility` (extra "li" — spotted via the migration FILENAME, since migration names derive from the model name); typo #2 = `related_name='availabilites'`. Admin JSON input gotcha: double quotes only. Commit `033db56`.
- **15c**: schedule setup + gate. **User upgraded breaks mid-build 1 → 2 → UNLIMITED**: final design = breaks are NOT form fields; template one `.break-row` with shared-name `type=time` inputs + 8-line cloneNode JS "+ Add break"; view `request.POST.getlist()` + zip → JSON. One-recurring-rule invariant (delete other non-DATE rows before save). Gate: DOCTOR login without recurring rule → `doctor_schedule`. Browser-verified full loop. Commits `e6a1b32` + `8badc31`.
- **Day 25 typos (6 total)** — model-class extra-syllable, related_name missing-letter, leftover form fields after design change, `form.erros`/`forms.erros`, `recureence` in exclude(), **NEW: return-outside-if** (mis-indented return sent ALL roles to doctor_today, dead code below — valid Python, invisible to `check`). Grep list += return-indentation in multi-branch views.
- **User question answered**: schedule saved but patient can't see it — correct, that's 15d (validation blocks bad bookings; cheap hours-text on booking page) + Step 20 (green/red calendar).
- Modes: caveman OFF, ponytail ON, Fable/Opus mix.

### Day 26 recap (2026-07-17) — STEP 15d SHIPPED, STEP 15 (avail+cancel) COMPLETE → PR #6

- **15d validation**: `_validate_doctor_available` helper (DATE-override-first two-lookup, no-schedule blocks, weekday check, `start <= slot < end` window, breaks via HH:MM string compare) called from BOTH booking forms after `_validate_slot_free`. Concept taught: time-vs-string comparison — model TimeFields compare as `time` objects, JSON breaks as zero-padded strings; never mix.
- **Hours text on book.html** — recurring rule in context, breaks rendered via dict dot-lookup. Poor-man's availability display until Step 20 calendar.
- **USER-REQUESTED mid-session — schedule view/update page**: schedule set → summary block + Update button only; `?edit=1` → form prefilled (`initial=` + break rows with values); save → redirect back to schedule page. Update = override (the 15c one-recurring-rule invariant already does it). No new view, no JS state.
- **Day 26 typos (3, all in one typed view/template)**: (1) `elif current:`/`else:`/`return render` mis-indented INSIDE the POST block → GET → `ValueError: view returned None` + failed-POST would have swapped the bound error form for a fresh one (return-indentation pattern, 2nd session running); (2) breaks JSON key `'ends'` instead of `'end'` → KeyError at booking validation — and STALE DATA lesson: rows saved during the bug carry the bad key, re-save to rewrite; (3) `forms.erros` (3rd occurrence across sessions).
- **Verified**: 4 validation cases + view/update loop + first-login gate still intact.
- Commits: `447496b` feat(appointments): validate book against doctor availability, `3da9012` feat(appointments): show doctor hours on booking + schedule view/update page.
- **STEP 15 (availability + cancel reason) COMPLETE (15a–15d)** → docs commit → **PR #6**.

### Day 27 recap (2026-07-18) — STEP 16a SHIPPED (lab models + media)

- **PR #6 was merged end of Day 26** (`57f4f71`); remote branch already deleted by user. NEW branch `feature/lab-module`.
- **16a shipped**: `lab` app scaffold + `LabTest`/`LabResult` models + admin + migration `lab.0001_initial` + media serving in dev. First `OneToOneField` (one result per test, schema-enforced; reverse = single object) + first `FileField` (path in DB, bytes in `MEDIA_ROOT/lab_results/`; Pillow NOT needed — ImageField-only). PROTECT on appointment/requested_by/uploaded_by (medical audit), CASCADE only test→result. `Meta.ordering=['requested_at']` = 16b queue order free. `static(MEDIA_URL, document_root=MEDIA_ROOT)` self-disables in production.
- **Browser-verified**: admin add test → upload result file → on disk + URL serves → duplicate result blocked.
- **Day 27 typos (2)**: `class status` lowercase (NameError at check — enum-vs-field casing at definition side, loud not silent), `ForeingKey` (Day-14 transposition family).
- **Commits**: `acd3f69` (lab app + settings), `f552368` (media serving) + docs commit.
- Modes: caveman full + ponytail full (user request, token saving).

### Day 28 resume point — Step 16b (doctor request-test + lab queue)

1. Confirm on `feature/lab-module`, clean tree, synced.
2. **16b scope (~1–1:15)**: doctor requests a test from a CONFIRMED appointment + lab technician queue + LAB role wiring.
   - `lab/forms.py`: small form (test_name only) OR just a text input + POST view (ponytail: no ModelForm needed for one field — decide at build).
   - `lab/views.py`: `request_test` (doctor-only, appointment must be CONFIRMED + doctor=me, creates LabTest with requested_by=me) + `lab_queue` (LAB-role-gated, `LabTest.objects.filter(status__in=['REQUESTED','IN_PROGRESS']).select_related('appointment__patient')` — oldest first via Meta.ordering).
   - `lab/urls.py` NEW (`app_name='lab'`) + mount in config.
   - Request-test button/input on doctor pages (CONFIRMED rows — doctor_today + history).
   - LAB login redirect → `lab:queue` + LAB nav branch in base.html.
   - Concept: `status__in` lookup, deeper select_related chain (`appointment__patient`).
3. Then 16c upload+status, 16d ReportLab PDF → PR #7.
4. Grep list active: enum-vs-field casing (both sides now), transposed model attrs, return-indentation, JSON keys, forms.erros, context-dict keys.
