...
**Day 33 (2026-07-24) — Step 17c shipped.** Patient prescription view + doctor-page column cleanup. `view_prescription` (`get_object_or_404(Prescription, appointment__id=id, appointment__patient=request.user)` — cross-FK lookup = IDOR guard in one query) + `prescriptions:view` route + read-only `view.html` (loops `medicines` JSON into table + diagnosis/advice) + COMPLETED-row link on `my_appointments`. **Doctor read-back built nothing** — 17b write button already loads the existing prescription, so read = the write page. UX turn (Prince's call): split the doctor `Action` column into **Action / Tests / Prescription** on both `doctor_records` + `doctor_today`. Perf: `appt.prescription` reverse OneToOne → `'prescription'` in `select_related` on both queries (reverse FK `lab_tests` stays prefetch). Zero bugs.
...
## Day 35 resume point — Step 18 notifications + background tasks

1. **New session** — `/clear`; point Claude at this resume + `PROJECT_LOG.md` Days 32–34 only.
2. **First: merge Step 17.** Commit 17d (`prescriptions/view.html`), open + merge PR for `feature/prescriptions`, then branch `feature/notifications` off `main`.
3. **Step 18** — Notification model + 8 triggers (see Notification matrix above) + bell + email. **DECIDE: Celery+Redis vs Django 6 built-in Tasks** (lean built-in) for the 24-hr reminder + auto-expire stale PENDING → CANCELLED. NOTE: never auto-complete CONFIRMED (confirmed ≠ visited). Lab notifications (test requested → LAB; result uploaded → patient+doctor) live here too. BIG step → scope-check + split into sub-steps (18a…) before code.
