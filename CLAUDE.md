...
- [ ] **Step 13** — `appointments` app (models with foreign keys, ORM queries) — **BIG step, chunked 13a → 13d, single branch `feature/appointments-app`, single PR #3 at end (Option A)**
  ...
  - [ ] **Step 13b** — Patient-side booking views + URLs + templates
  - [ ] **Step 13c** — Doctor view (today's appointments, cancel) + role-based redirect override on login
...
### Day 15 resume point — Step 13b (patient-side booking)

1. Open Claude Code. CLAUDE.md auto-loads. Confirm on `feature/appointments-app` with 4 commits (3 feat + 1 docs). Working tree clean. `git pull` to sync.
2. **Pre-session scope check** — propose 13b plan, get user confirm before code typed (chunking rule):
   - **13b scope**: patient-side booking flow — list doctors page + booking form + "my appointments" list + cancel button.
   - **Concepts**: QuerySets (`filter`/`exclude`/`get`), `ModelForm` with FK fields, dynamic queryset for doctor dropdown (`CustomUser.objects.filter(role='DOCTOR')`), `select_related` for N+1 prevention, `@login_required` per-role gate.
   - **Files**: `backend/apps/appointments/views.py` (4 views), `backend/apps/appointments/urls.py` (NEW), wire into `backend/config/urls.py`, `backend/apps/appointments/forms.py` (NEW), templates `appointments/doctor_list.html` + `appointments/book.html` + `appointments/my_appointments.html`. ~5-6 files.
   - **Estimated time**: 1:15–1:30 hr. If runs over → hard stop, push WIP, finish in 13b-continuation.
3. **Teach concepts FIRST** per [[feedback-teaching-mode]] (expert-teacher voice, real Django terms + worked examples + "why it matters") + animate mechanic per Day 10 teaching adjustment.
4. Code → browser verify → commits.
5. **Per [[feedback-incremental-tutorial]]**: write 13b tutorial section (append to `tutorial/13-appointments-app.md` OR new file `13b-appointments-views.md` — decide convention at start) + update README index to `⏳ In progress (13a + 13b done)` + roadmap 13b checked + memory recap + docs commit. THEN stop session.
6. PR #3 NOT opened yet — waits for 13d (Option A).
