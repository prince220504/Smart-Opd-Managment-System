...
- [ ] **Step 13e** — Appointment lifecycle: `COMPLETED` status (CONFIRMED→COMPLETED, doctor own / reception any, Complete button on CONFIRMED rows, needs migration) + doctor time-split views (history = `appointment_date__lte=today`, upcoming = `__gt=today`). NEW branch (`feature/appointments-lifecycle`) → PR #4. Likely split 13e-i (COMPLETED status) + 13e-ii (history/upcoming). Concepts: `__lte`/`__gt` field lookups, terminal-state guards, new status transition. User-requested Day 17.
- [ ] **Step 14** — DRF layer (serializers, viewsets, routers, JWT) — deferred per user (Day 17), do after Step 13e
...
### Day 18 resume point — PR #3 then Step 13e (new branch)

1. **If PR #3 not yet merged**: open Claude Code on `feature/appointments-app`. Open PR #3 (`gh pr create` or web UI), title `feat(appointments): appointment booking — patient + doctor + reception (Step 13)`. Review own diff → merge → `git checkout main && git pull` → delete branch local+remote → confirm `git branch -a` clean.
2. **Step 13e** on NEW branch `feature/appointments-lifecycle` from `main`. Pre-session scope check (chunking rule — BIG, likely split 13e-i + 13e-ii):
   - **13e-i — COMPLETED status**: add `COMPLETED = 'COMPLETED', 'Completed'` to `Appointment.Status`; `makemigrations` + `migrate` (choices change → migration). `complete_appointment` view (`@require_POST`, `CONFIRMED → COMPLETED` guard, reception any / doctor own — mirror confirm). Route `complete/<int:id>/`. Complete button on CONFIRMED rows (doctor_today + appointment_list). Lifecycle terminal states: COMPLETED + CANCELLED.
   - **13e-ii — doctor time-split views**: `doctor_history` (`doctor_appointments.filter(appointment_date__lte=today).select_related('patient')`, past+today) + `doctor_upcoming` (`__gt=today`, future). Routes + templates + doctor nav links. Concepts: `__lte`/`__gt` field lookups.
   - Optional carryover: login/register already-authenticated guard (deferred since 13c).
3. Per [[feedback-incremental-tutorial]]: tutorial + docs commit per sub-step. New tutorial file `tutorial/13e-appointments-lifecycle.md` (or append — decide at start). After 13e done → PR #4.
4. Maintain expert-teacher voice + animated mechanic + pre-commit grep list.
