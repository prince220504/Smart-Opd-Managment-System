...
**Day 34 (2026-07-25) — Step 17d shipped, Step 17 CLOSED.** Prescription "PDF" with **zero server code**. Chose `window.print()` + `@media print` over ReportLab (the rendered HTML *is* the doc; no library/view/route/layout — unlike lab 16d which cut its PDF for a different reason). `<style>` hides `header, footer, .no-print` on print; `<button type="button" onclick="window.print()">` → browser "Save as PDF". Default save filename = `document.title`, so title block rebuilt to `Prescription -{patient.username}-{appointment_date|date:"Y-m-d"}` (browser only suggests, user can rename). Also closed a 17c gap: `view.html` never rendered `advice` → added `{% if prescription.advice %}` block. 3 silent template typos Prince caught on test: `prescrition.advice` (missing `p` → blank text under a correctly-spelled `{% if %}` heading), `prescription.appointment_date` in title (dropped `.appointment` hop → empty filename segment). All template-only, no Python errors, pure empty renders. Only file: `prescriptions/view.html`. **Step 17 done → open PR for `feature/prescriptions`.**
...
## Day 36 resume point — Step 18b in-app triggers

1. **New session** — `/clear`; point Claude at this resume + `PROJECT_LOG.md` Days 34–35 only.
2. **18b — wire the 6 in-app triggers** into existing views via a `notify(recipient, message, type, link)` helper (build the helper in `notifications/` first — it's the shared write path; one guard, all callers route through). Triggers: booking confirmed→patient · test requested→lab tech · result uploaded→patient+doctor · cancelled→doctor · confirmed→patient · prescription written→patient · walk-in→reception. In-app only (`Notification.objects.create`), **no email yet** (email = 18c). Touch appointments (book/confirm/cancel/walk-in) + lab (request_test/upload_result) + prescriptions (write) views.
3. **Then 18c** bell UI + email (Gmail SMTP), **18d** Celery+Redis (24-hr reminder + auto-expire stale PENDING). Never auto-complete CONFIRMED (confirmed ≠ visited).
