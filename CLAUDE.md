...
- [ ] **Step 14** — DRF layer (serializers, viewsets, routers, JWT) — deferred per user (Day 17), do after Step 13e. **PLUS (user-requested Day 19): double-booking fix** — `UniqueConstraint(doctor, appointment_date, time_slot)` with `condition=~Q(status='CANCELLED')` in `Appointment.Meta` + migration + friendly ValidationError in both booking forms. DB constraint over app code (race-proof).
...
### Day 20 resume point — merge PR #4 → Step 14 (DRF, new branch)

1. **If PR #4 not merged yet**: open on `feature/appointments-lifecycle` — title `feat(appointments): appointment lifecycle — completed/no-show + doctor history/upcoming (Step 13e)`. Review own diff → merge → `git checkout main && git pull` → delete branch local+remote.
2. **Step 14 — DRF layer** on NEW branch (`feature/drf-api` or similar). BIG step — pre-session scope check + sub-step split proposal first. Contents: serializers, viewsets, routers, SimpleJWT auth, drf-spectacular docs, django-filter. **PLUS double-booking `UniqueConstraint`** (Day 19 deferral — could be 14a since it's model-layer).
3. Optional carryover: login/register already-authenticated guard (deferred since 13c).
4. Teaching: expert-teacher voice + animated mechanic + concept-checks. Pre-commit grep list now includes: context-dict keys, form method/action swap.
