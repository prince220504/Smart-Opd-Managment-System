...
- [ ] **Step 16 — lab module** — branch `feature/lab-module` → PR #7. `lab` app scaffold; `LabTest` model (appointment FK, requested_by, test_name, status REQUESTED/IN_PROGRESS/DONE, requested_at); `LabResult` model (OneToOne LabTest, uploaded_by, result_file FileField, notes, is_normal, result_date); media serving in dev; doctor request-test (CONFIRMED appts only); lab pending queue (oldest first) + LAB role login redirect + nav; upload result + status flow; ReportLab branded PDF report. Likely split 16a models+admin+media / 16b request+queue / 16c upload+status / 16d PDF. Auth extras (forgot-password via built-in PasswordResetView, profile photo w/ Pillow) — separate sub-step or own mini-branch.
...
### Day 27 resume point — merge PR #6 → lab module (new branch)

1. **PR #6**: on `feature/availability-and-cancel-reason` — title `feat(appointments): doctor availability + booking validation + cancel reason (Step 15)`. Review → merge → `git checkout main && git pull` → delete branch local+remote.
2. **Lab module** on NEW branch `feature/lab-module`. BIG — pre-session scope check + split FIRST. Content (catalogue): `lab` app scaffold; `LabTest` model (appointment FK, requested_by, test_name, status REQUESTED/IN_PROGRESS/DONE, requested_at); `LabResult` model (OneToOne LabTest, uploaded_by, result_file FileField, notes, is_normal, result_date); media serving in dev; doctor request-test (CONFIRMED appts only); lab pending queue (`/lab/` dashboard, oldest first); lab upload result + status flow; ReportLab PDF branded report; LAB role login redirect + nav. Plus auth extras (forgot-password built-in, profile photo + Pillow) — possibly separate sub-step or branch.
3. Likely split: 16a models+admin+media / 16b doctor request + lab queue / 16c upload + status / 16d PDF. **Numbering FIXED Day 26 (user request): lab = Step 16.** Full sequence: 16 lab / 17 prescriptions / 18 notifications+background-tasks / 19 dashboard+exports / 20 deploy / 21 frontend (green/red calendar rides with frontend → Step 21). Commits use `step-16x` prefixes.
4. Modes carry: caveman ON(for less token uses), ponytail ON. Grep list active (+return-indentation ×2, JSON key typos, stale-data-after-bug).
