...
- [ ] **Step 14** — DRF layer, chunked 14a → 14d, branch `feature/drf-api` → PR #5
  ...
  - [ ] **Step 14c** — Role-scoped `AppointmentViewSet` (get_queryset: patient own / doctor own / reception all) + `DefaultRouter` under `/api/` + `IsAuthenticated` + browsable API. Concepts: ViewSet vs FBV, router vs urlpatterns, content negotiation.
  - [ ] **Step 14d** — SimpleJWT auth (`/api/token/` + refresh) + write ops via API + django-filter decision + drf-spectacular `/api/docs/`.
...
### Day 22 resume point — Step 14c (ViewSet + router)

1. Confirm on `feature/drf-api`, clean tree, synced. `OPD_PROJECT_STATUS.md` present locally (gitignored) — read for full feature map.
2. **14c scope (~1:15)**: role-scoped `AppointmentViewSet(viewsets.ModelViewSet)` — `get_queryset()` branches on `request.user.role` (patient→own via patient_appointments, doctor→own via doctor_appointments, reception→all). `perform_create` sets patient for patient-role. `DefaultRouter` registered under `/api/` in a new `api_urls` include (or config/urls). `IsAuthenticated` default permission. Browsable API verify at `/api/appointments/`.
3. Concepts: ViewSet vs FBV (CRUD from one class), router vs urlpatterns (auto-generated routes), the browsable API, content negotiation.
4. Reuse the role-scoping logic pattern from the HTML views (don't re-derive — same patient/doctor/reception branching).
5. Per [[feedback-incremental-tutorial]]: 14c tutorial + docs commit. Then 14d (JWT + write ops + filters), 14e (spectacular), then PR #5.
6. Carryover decision for Step 15: TimeSlot/availability model — build or keep field-based (flagged in catalogue).
