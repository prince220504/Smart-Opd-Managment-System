...
- [ ] **Step 18 — notifications + background tasks** 🚧 — Notification model + 8 triggers (matrix) + bell + email. **DECIDED (Day 35): Celery + Redis** (Prince's call over Django 6 Tasks) for 24-hr reminder + auto-expire stale PENDING → CANCELLED. NOTE: never auto-complete CONFIRMED (confirmed ≠ visited). Sub-step split: 18a model · 18b in-app triggers · 18c bell UI + email (Gmail SMTP) · 18d Celery/Redis background tasks.
...
**Day 35 (2026-07-26) — Step 18a shipped.** Branch `feature/notifications` off `main`. `notifications` app + `Notification` model — the generic "system told user X something" row: `recipient` FK→`settings.AUTH_USER_MODEL` CASCADE `related_name='notifications'` (= the `user.notifications.all()` bell query), `message` CharField(255), `notification_type` TextChoices (6 categories — the 8 matrix events collapse to BOOKING/REMINDER/TEST/RESULT/PRESCRIPTION/STATUS; category drives icon not logic), `link` **plain CharField** (stored resolved-URL string, not FK — one column for many target page types), `is_read` (badge count), `created_at`, `Meta.ordering=['-created_at']`. Admin with `recipient__username` FK-span search + `is_read`/type filters. Migration `0001`. **Decision this session:** background engine = **Celery + Redis** (Prince's call, I argued Django 6 Tasks once then landed his). Data-layer only — no triggers/bell/email. Zero typos.

**Day 36 (2026-07-27) — Step 18b shipped.** `notifications/services.py` with `notify(recipient, message, notification_type, link='')` — the **single write path** every trigger routes through (chosen over 7× inline `objects.create`, and over signals: a `post_save` fires on *every* save incl. admin edits/imports → no control over which status change means what). Helper takes a **recipient object not an id** (caller already holds it → zero extra query on FK assign) and deliberately has **no `notification_type` default** even though the model field does (model default protects anonymous writers; helper is stricter because every trigger knows its own type — missing arg should crash, not silently save STATUS). 7 triggers wired: book→patient · reception_book→patient · confirm→patient · cancel→**other party** · request_test→**all LAB users** · upload_result→patient **+** doctor · write_prescription→patient. Cancel's "other party" = `[patient, doctor]` + `if user != request.user` — 2 lines covering all 3 canceller roles (patient→doctor, doctor→patient, reception→both), safe because Django `Model.__eq__` compares **PKs** and proxies through `request.user`'s `SimpleLazyObject`. Lab loop writes **one row per tech** (shared row can't track per-user `is_read`); upload_result uses **2 explicit calls not a loop** (recipients need different links). `reverse()` taught as Python `{% url %}`, `args=[...]` for URL params — **`manage.py check` does NOT catch `NoReverseMatch`**, so all 5 names were resolved in a shell before trusting them. **Core rule: `notify()` goes after `.save()` and INSIDE the state guard** — that guard is the idempotency check; outside it a double-clicked Confirm or refreshed POST writes duplicates (valid Python, wrong behaviour). One typo: `appointment.patent.username` → `AttributeError` firing *after* the result file saved, status flipped DONE, and the patient's notification was already written = half-done write (real cure `transaction.atomic()`, later). Accepted duplicate: `write_prescription` is create-or-update so an edit re-notifies (noise not bug; judge it in 18c when the bell makes it visible). Tested 4/7 via a shell dump (admin table too clumsy without UI) incl. the branchy cancel — verified reception-cancel correctly writes 2 rows; remaining 4 are straight-line and verify free once the bell renders.

## Day 37 resume point — Step 18c bell UI + email

1. **New session** — `/clear`; point Claude at this resume + `PROJECT_LOG.md` Days 35–36 only.
2. **18c — bell + email.** Bell in `base.html` nav: unread count + dropdown of recent, `user.notifications.all()` (the `related_name` from 18a), mark-as-read view, click → `notification.link`. Then Gmail SMTP (app password in `.env`, `EMAIL_*` settings) and add an `email=False` flag to `notify()` — **only 2 of the 7 get mail** per the matrix (booking confirmed→patient, result uploaded→patient+doctor); the flag is what stops the shared helper emailing all 7. While the bell is live, eyeball the 4 untested triggers (walk-in, request_test, upload_result, write_prescription) and decide the write_prescription duplicate.
3. **Then 18d** Celery+Redis (24-hr reminder + auto-expire stale PENDING). Never auto-complete CONFIRMED (confirmed ≠ visited).
