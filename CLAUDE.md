...
**Day 36 (2026-07-27) — Step 18b.** `notifications/services.py` `notify(recipient, message, notification_type, link='')` = **single write path** (beat 7× inline `create` and signals — `post_save` fires on every save incl. admin). Takes recipient **object** not id (no extra query); **no type default** (model has one; helper stricter — missing arg should crash). 7 triggers: book · reception_book · confirm → patient · cancel → **other party** (`[patient,doctor]` + `if user != request.user` = all 3 canceller roles; `Model.__eq__` compares PKs, safe vs SimpleLazyObject) · request_test → **all LAB users** (one row each — `is_read` is per-row) · upload_result → patient **+** doctor (2 calls not loop, different links) · write_prescription → patient. `reverse()` = Python `{% url %}`; **`check` misses `NoReverseMatch`** → resolved all 5 in shell. **Rule: `notify()` after `.save()`, INSIDE the state guard** (guard = idempotency; outside it a refreshed POST double-writes). Typo `appointment.patent` → AttributeError *after* file saved + 1st notify = half-done write (cure: `transaction.atomic()`, later). Tested 4/7 by shell dump.
...
## Day 38 resume point — Step 18d Celery + Redis + email

1. **New session** — `/clear`; point Claude at this resume + `PROJECT_LOG.md` Days 36–37 only.
2. **Email** (moved from 18c). Gmail SMTP: app password in `.env`, `EMAIL_*` settings, then an `email=False` flag on `notify()` in `services.py` — **only 2 of the 7 get mail** per the matrix (booking confirmed→patient, result uploaded→patient+doctor). The flag is what stops the shared helper mailing all 7. Test with `console.EmailBackend` first.
3. **18d Celery + Redis** — 24-hr reminder + auto-expire stale PENDING → CANCELLED. Never auto-complete CONFIRMED (confirmed ≠ visited).
