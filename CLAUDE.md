...
### Day 16 resume point — Step 13c (doctor view + role-based redirect)

1. Open Claude Code. CLAUDE.md auto-loads. Confirm on `feature/appointments-app` with 7 commits (5 feat + 2 docs after this session's docs commit lands). Working tree clean. `git pull` to sync.
2. **Pre-session scope check** — propose 13c plan, get user confirm before code typed (chunking rule):
   - **13c scope**: doctor-side view (today's appointments list + doctor-cancel action) + role-based redirect after login.
   - **Concepts**: `request.user.doctor_appointments` reverse FK (mirror of `patient_appointments` used in 13b), `filter(appointment_date=today)` + `datetime.date.today()`, role-based dispatch on login (`if user.role == 'DOCTOR': redirect(...)`), override `LOGIN_REDIRECT_URL` strategy vs custom `login_view` branching.
   - **Files**: `backend/apps/appointments/views.py` (2 new views: `doctor_today`, `doctor_cancel`), `backend/apps/appointments/urls.py` (2 new routes under `/doctor/` namespace OR keep `/appointments/doctor/...`), `backend/apps/accounts/views.py` (modify `login_view` to branch on role), templates `appointments/doctor_today.html`. ~3-4 files.
   - **Estimated time**: 1:00–1:15 hr.
3. **Teach concepts FIRST** per [[feedback-teaching-mode]] + Day 10 animated-mechanic rule.
4. Code → browser verify (login as DOCTOR, see today's appointments page) → 2-3 atomic commits.
5. **Per [[feedback-incremental-tutorial]]**: write 13c tutorial section (append to `13-appointments-app.md`) + update README index to `⏳ In progress (13a + 13b + 13c done)` + roadmap 13c checked + memory recap + docs commit `docs(step-13c): ...`. THEN stop session.
6. PR #3 STILL not opened — waits for 13d (Option A).
