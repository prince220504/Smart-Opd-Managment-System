...
**Day 32 (2026-07-23) — Step 17a + 17b shipped (both same day).** Branch `feature/prescriptions`. **17a**: `prescriptions` app + `Prescription` model — OneToOne appointment (CASCADE, reverse `appointment.prescription`) + **first project `JSONField`** `medicines` (`default=list` = list-of-dicts in one column, no child table) + diagnosis/advice/created_at + admin + migration `0001`. COMPLETED-gate is a view rule, not a model constraint. **17b**: doctor write page — `PrescriptionForm` (diagnosis/advice only) + `write_prescription` view (`get_object_or_404(Appointment, id, doctor=me, status=COMPLETED)` = auth + COMPLETED-gate in the lookup; `instance=getattr(appt,'prescription',None)` = create-or-update, dodges OneToOne IntegrityError like 16c) + medicine rows via `getlist`×4 + `zip` + `if name:` → JSON + `cloneNode` "+ Add medicine" + `prescriptions:write` route (config include) + COMPLETED-row button on doctor_records (`{% if appt.prescription %}` flips Write/View-Edit label, safe reverse-OneToOne read). Only bug: `getElementByID` → `getElementById` (JS case-sensitive; clone built but append threw → button silently dead).

**Day 33 (2026-07-24) — Step 17c shipped.** Patient prescription view + doctor-page column cleanup. `view_prescription` (`get_object_or_404(Prescription, appointment__id=id, appointment__patient=request.user)` — cross-FK lookup = IDOR guard in one query) + `prescriptions:view` route + read-only `view.html` (loops `medicines` JSON into table + diagnosis/advice) + COMPLETED-row link on `my_appointments`. **Doctor read-back built nothing** — 17b write button already loads the existing prescription, so read = the write page. UX turn (Prince's call): split the doctor `Action` column (was lifecycle + Request Test + test list + prescription button all jammed together) into **Action / Tests / Prescription** on both `doctor_records` + `doctor_today` (latter gained a Prescription column). Perf: `appt.prescription` is a reverse OneToOne → added `'prescription'` to `select_related` on both patient + doctor_today queries (reverse OneToOne joins; reverse FK `lab_tests` stays prefetch). Decided: Request Test stays on CONFIRMED (doctor orders live during consult, optional), not COMPLETED. Zero bugs.

## Day 34 resume point — Step 17d prescription PDF

1. **New session** — `/clear`; point Claude at this resume + `PROJECT_LOG.md` Day 33 only.
2. **Still on branch `feature/prescriptions`** (check `git status`; commit pending 17c first — user commits himself).
3. **Step 17d** — prescription PDF/print. Decide ReportLab-vs-direct like 16d, but **content is generated here** (diagnosis/medicines/advice, no uploaded file) → a real PDF (or browser print-CSS) likely stays this time, unlike the lab report. Cheapest option first: a print-friendly template + `window.print()` may beat ReportLab entirely — evaluate before installing anything.
4. After 17d → **Step 17 closes**, open PR for `feature/prescriptions`.
5. Lab notifications (test requested → LAB; result uploaded → patient+doctor) still deferred to **Step 18**.
