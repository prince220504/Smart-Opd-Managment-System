...
### Day 23 resume point — Step 14d (SimpleJWT + write ops + filters)

1. Confirm on `feature/drf-api`, clean tree, synced.
2. **14d scope (~1:15)**: `pip install djangorestframework-simplejwt` + freeze + settings (`REST_FRAMEWORK` `DEFAULT_AUTHENTICATION_CLASSES` += JWTAuthentication, keep SessionAuthentication for browsable API). `TokenObtainPairView` (`/api/token/`) + `TokenRefreshView` (`/api/token/refresh/`) in config or api/urls. Test write ops via API (POST create as patient → perform_create injects patient; PATCH/DELETE scoped). django-filter DECISION: install + FilterSet for the API list, OR keep manual (the HTML list already filters manually — YAGNI leans keep-manual, but API is the natural place for django-filter; decide at build).
3. Concepts: JWT vs session auth (stateless token vs server session), access/refresh token pair, Authorization: Bearer header.
4. Then 14e (drf-spectacular `/api/docs/` Swagger) → PR #5 closes Step 14.
5. Carryover Step 15 decision: TimeSlot/availability model — build or keep field-based.
6. Reminder: reconcile CLAUDE.md Step 16 (Celery) vs catalogue's Django-6-built-in-Tasks when reaching notifications.
