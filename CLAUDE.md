...
2. **Pre-session scope check** — propose 13d plan, confirm before code (chunking rule):
   - **13d scope**: receptionist book-on-behalf form (sets `patient=<any user>` — the field patients/doctors can't set) + global appointment list (all appointments, not ownership-scoped) + filters (by status / date / doctor).
